    ...
    <div class="col-md-12 table-responsive">
      <table id="datatables" class="table table-bordered table-striped text-center">
        ...
        <tbody>
          ...
          <tr>
            <th scope="row"><a href="/customers/reports/progress-report/detail/{{$pr->first()->report_code}}" class="btn btn-success btn-sm"><span><i class="fas fa-eye"></i></span></a></th>
            <td>{{$pr->first()->report_code}} - {{date('d F Y', strtotime($pr->first()->report_date))}} <br>
              <a href="/customers/reports/progress-report/detail/{{$pr->first()->report_code}}">Laporan Progress {{$pr->first()->Projects->project_name}} - Programmers {{$pr->first()->Programmers->name}}</a></td>
